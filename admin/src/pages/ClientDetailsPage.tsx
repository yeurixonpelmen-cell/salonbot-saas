...
export function ClientDetailsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setTelegramId(clientData.telegram_id ? String(clientData.telegram_id) : '');
  ...
  async function submit(event: FormEvent) {
    ...
    setError('');
    try {
      const parsedTelegram = telegramId.trim() ? Number(telegramId.trim()) : null;
      await api.patch<Client>(`/api/admin/clients/${id}`, {
        ...
        telegram_id: parsedTelegram && Number.isFinite(parsedTelegram) ? parsedTelegram : null,
  ...
  async function removeClient() {
    if (!client) return;
    if (!window.confirm(`Видалити картку «${client.full_name}»? Записи залишаться без прив’язки.`)) return;
    setDeleting(true);
    setError('');
    try {
      ...
      setError((err as { error?: string }).error ?? 'Не вдалося видалити клієнта');
      setDeleting(false);
    }
  }
  ...
  return (
    <div className="page-stack">
      ...
      <header className="client-profile-header">
        ...
        <div>
          <span className="eyebrow">Картка клієнта</span>
      ...
      <div className="client-details-grid">
        <section className="content-card">
          ...
          <form className="form-grid" onSubmit={submit}>
            ...
            <label>Telegram ID<input value={telegramId} onChange={(e) => setTelegramId(e.target.value)} placeholder="напр. 123456789" inputMode="numeric" /></label>
            ...
            <div className="form-actions full">
              ...
              <Button type="button" variant="danger" disabled={deleting} onClick={() => void removeClient()}>
                {deleting ? 'Видалення…' : 'Видалити клієнта'}
              </Button>
            </div>
          </form>
        </section>
            ...
            {files.map((file) => {
              ...
              return (
                <div className="file-row" key={file.id}>
                  ...
                  <div>
                    <b>{fileName(file)}</b>
                    <small>{file.size_bytes ? `${(file.size_bytes / 1024 / 1024).toFixed(1)} МБ` : 'Файл'}</small>
                  </div>
                  {href && <a href={href} target="_blank" rel="noreferrer">Відкрити</a>}
                  <button type="button" onClick={() => void removeFile(file)}>Видалити</button>
                </div>
              );
            })}
        ...
        <div className="visit-history">
          {client.bookings?.map((booking) => (
            <div className="visit-row" key={booking.id}>
              <time>
                {new Date(booking.datetime).toLocaleDateString('uk-UA', { day: '2-digit', month: 'short', year: 'numeric' })}
                <small>{new Date(booking.datetime).toLocaleTimeString('uk-UA', { hour: '2-digit', minute: '2-digit' })}</small>
              </time>
