...
function mapFinanceEntry(row: any) {
...
router.get('/admin/finance', async (req: Request, res: Response) => {
  ...
  let entries = (data ?? []).map(mapFinanceEntry);
  if (search) {
  ...
  const net = Math.round((income_sum - expense_sum) * 100) / 100;
  const tax_5pct = Math.round(income_sum * 0.05 * 100) / 100;
...
router.post('/admin/finance', async (req: Request, res: Response) => {
  const salonId = req.auth!.salon_id!;
  const {
    entry_date,
    kind,
  ...
  if (master_id) {
    const { data: master } = await supabase
      ...
      .maybeSingle();
    if (!master) {
      res.status(400).json({ error: 'Спеціаліста не знайдено' });
      return;
    }
    masterId = master.id;
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      master_id: masterId,
      booking_id: typeof booking_id === 'string' && booking_id ? booking_id : null,
  ...
  res.status(201).json(mapFinanceEntry(data));
});

router.patch('/admin/finance/:id', async (req: Request, res: Response) => {
  const salonId = req.auth!.salon_id!;
  const patch: Record<string, unknown> = {};
  ...
  if (req.body.master_id !== undefined) {
    if (!req.body.master_id) {
      patch.master_id = null;
    } else {
      const { data: master } = await supabase
        .from('masters')
        .select('id')
        .eq('id', req.body.master_id)
        .eq('salon_id', salonId)
        .maybeSingle();
      if (!master) {
        res.status(400).json({ error: 'Спеціаліста не знайдено' });
  ...
  res.json(mapFinanceEntry(data));
});

router.delete('/admin/finance/:id', async (req: Request, res: Response) => {
  const { error } = await supabase
    .from('finance_entries')
    .delete()
    .eq('id', req.params.id)
...
router.post('/admin/finance/import-bookings', async (req: Request, res: Response) => {
  const salonId = req.auth!.salon_id!;
  const from = typeof req.body?.from === 'string' ? req.body.from.trim() : '';
  const to = typeof req.body?.to === 'string' ? req.body.to.trim() : '';
  const method = FINANCE_METHODS.includes(req.body?.payment_method)
    ? req.body.payment_method
    : 'iban';
  if (!from || !to) {
    res.status(400).json({ error: 'Потрібен період from / to' });
    return;
  }
  ...
  let dayEnd: string;
  try {
    dayStart = dayRangeUtc(from, timeZone).startIso;
    dayEnd = dayRangeUtc(to, timeZone).endIso;
  } catch {
    res.status(400).json({ error: 'Некоректний період' });
    return;
  }
  ...
  const { data: bookings, error } = await supabase
    .from('bookings')
    .select(
      'id, booking_datetime, client_name, master_id, status, visit_status, services(name_uk, price), masters(name)'
    )
    .eq('salon_id', salonId)
    .neq('status', 'cancelled')
    .gte('booking_datetime', dayStart)
    .lte('booking_datetime', dayEnd);
  ...
  const linked = new Set((existing ?? []).map((row) => row.booking_id).filter(Boolean));

  const rows = [];
  for (const booking of bookings ?? []) {
    if (linked.has(booking.id)) continue;
    if (booking.status !== 'completed' && booking.visit_status !== 'completed') continue;
    const service = Array.isArray(booking.services) ? booking.services[0] : booking.services;
    const master = Array.isArray(booking.masters) ? booking.masters[0] : booking.masters;
    const price = Number(service?.price);
    if (!Number.isFinite(price) || price <= 0) continue;
    const entryDate = zonedDateKey(booking.booking_datetime, timeZone);
    rows.push({
      salon_id: salonId,
      ...
      description: service?.name_uk ?? 'Послуга',
      master_id: booking.master_id ?? null,
      booking_id: booking.id,
      notes: master?.name ? `З запису · ${master.name}` : 'З запису',
    });
  }

  if (!rows.length) {
    res.json({ imported: 0, message: 'Немає завершених записів із ціною для імпорту' });
    return;
  }
  ...
  res.json({ imported: inserted?.length ?? rows.length });
});
